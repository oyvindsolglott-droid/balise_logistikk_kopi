...
(()=>{
  ...
  function run(order){
    ...
    const unresolvedSnapshot=snapshot([candidate],arranged);
    const unresolvedPlan=ctx.buildSdeCanonicalPlan({
      actualSources:unresolvedSnapshot.actualSources,
    ...
    return {
      ...
      unresolvedOccupiedTarget:{
        activeOutcomes:(unresolvedPlan.activeOutcomes || []).filter(item=>item.vehicleId==="74-11" && item.targetSlot==="4N").length,
        diagnostics:(unresolvedPlan.diagnostics || []).filter(item=>item.code==="target_physically_occupied" && item.vehicleId==="74-11" && item.targetSlot==="4N").length
      }
    };
  }
  ...
  put("INV-TARGET-008",observed.chainIds.length===1 && observed.rootRequestIds.length===1 && observed.planRevisions.length===1 && !observed.diagnosticTypes.includes("IMPOSSIBLE_SHIFT_TO_OR_FROM_BLOCKED_SLOT") && observed.unresolvedOccupiedTarget.activeOutcomes===0 && observed.unresolvedOccupiedTarget.diagnostics===1,"the complete occupied-target chain survives atomically while an unresolved occupied target still fails closed");
