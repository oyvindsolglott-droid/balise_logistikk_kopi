...
assert.doesNotMatch(
  source,
  /driftsklarAt/,
  "legacy local DROPS readiness must have no remaining production authority"
);
assert.doesNotMatch(
...
assert.match(
  extractFunction(source, "getSdePhysicalAvailabilityForVehicle"),
  /if\(dropsOrder && !workshopReady\)/,
  "an active DROPS/workshop flow must stay blocked until explicit workshop readback"
);
assert.match(
